...
class UsersService(BaseService):
    ...
    @staticmethod
    def create(login, email, tel_number, password_hash, first_name, last_name, role):
        query = "SELECT * FROM create_user(%s, %s, %s, %s, %s, %s, %s);"
        params = [login, email, tel_number, password_hash, first_name, last_name, role] 
        return database.execute(query, params=params, fetch_count=1)
    
    @staticmethod
    def update(id: int, email, tel_number, password_hash, first_name, last_name, role):
        query = "SELECT * FROM update_user(%s, %s, %s, %s, %s, %s, %s);"
        params = [id, email, tel_number, password_hash, first_name, last_name, role]
        return database.execute(query, params=params, fetch_count=1)
