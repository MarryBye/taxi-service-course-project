from src.controllers.database import Database
from src.schemas.orders import AdminCreateOrderSchema, AdminUpdateOrderSchema
from src.schemas.token import TokenDataSchema


class OrdersService:
    @staticmethod
    def list(limit: int = 10, offset: int = 0, user: TokenDataSchema = None):
        db = Database(user=user)
        query = "SELECT * FROM admin.list_orders(%s, %s)"
        params = [offset, limit]
        return db.execute(query, params=params, fetch_count=-1)

    @staticmethod
    def get(id: int, user: TokenDataSchema = None):
        db = Database(user=user)
        query = "SELECT * FROM admin.get_order(%s)"
        return db.execute(query, params=[id], fetch_count=1)

    @staticmethod
    def create(schema: AdminCreateOrderSchema, user: TokenDataSchema = None):
        db = Database(user=user)
        query = "CALL admin.create_order(%s, %s, %s, %s, %s, %s, %s)"
        params = [
            schema.client_id,
            schema.status,
            schema.order_class,
            schema.payment_method,
            schema.amount,
            schema.addresses,
            schema.driver_id
        ]
        return db.execute(query, params=params, fetch_count=1)

    @staticmethod
    def update(id: int, schema: AdminUpdateOrderSchema, user: TokenDataSchema = None):
        db = Database(user=user)
        query = "CALL admin.update_order(%s, %s, %s)"
        params = [
            id,
            schema.status,
            schema.order_class
        ]
        return db.execute(query, params=params, fetch_count=1)

    @staticmethod
    def delete(id: int, user: TokenDataSchema = None):
        db = Database(user=user)
        query = "CALL admin.delete_order(%s)"
        return db.execute(query, params=[id], fetch_count=0)

    @staticmethod
    def cancel(id: int, user: TokenDataSchema = None):
        db = Database(user=user)
        query = "CALL admin.cancel_order(%s)"
        return db.execute(query, params=[id], fetch_count=0)